...
contract ERCiobToken {
    ...
    function buyToken(uint256 amount) external payable returns(bool) {
        require(msg.value == amount.mul(tokenPrice), "Provided funds must be equal to the value of the tokens you want to purchase.");
    ...
    function transfer(address recipient, uint256 amount) external returns(bool) {
        require(block.number.sub(last_price_change_block) > transfer_timeout_period, "Token price has recently changed; transfers are momentarily disabled.");

        owned_tokens[msg.sender] = owned_tokens[msg.sender].sub(amount);
        owned_tokens[recipient] = owned_tokens[recipient].add(amount);

        // Taken from OpenZeppelin's "Address.sol" contract.
        uint256 size;
